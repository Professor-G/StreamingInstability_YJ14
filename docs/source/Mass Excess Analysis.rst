...
where :math:`a_\bullet` and :math:`\rho_\bullet` are the grain size and internal grain density. From the computed :math:`a_\bullet` an opacity from Birnstiel et al (2018) Figure 4 was calculated, although we noted that when using these opacities, the mass excess saturated. This was in part due to the inverse relation between opacity and :math:`\Sigma_d`, since 
